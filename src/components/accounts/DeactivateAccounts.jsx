...
const DeactivateAccounts = () => {
  ...
  const fetchAllAccounts = async () => {
    ...
    const deactivatedSnapshot = await getDocs(
      query(collection(db, "accounts"), where("isActivated", "==", false))
    ); //gets snapshot of all deactivated accounts
  ...
  const deactivateAccount = async (account) => {
    if (account.balance > 0) {
      ...
      await updateDoc(doc(db, "accounts", account.accountID), {
        isActivated: false,
      }); //deactivate account
